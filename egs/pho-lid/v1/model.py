...
class Dummy_Model(nn.Module):
    ...
    def forward(self, x, seq_len, atten_mask=None, label=1):
        line0 = torch.zeros(torch.Tensor(seq_len).shape).unsqueeze(0)
        line1 = torch.ones(torch.Tensor(seq_len).shape).unsqueeze(0)

        if label == 0:
            return torch.vstack((line0, line1))
        else:
            return torch.vstack((line1, line0))

    def get_embeddings(self, x, seq_len, atten_mask=None):
        return torch.Tensor(seq_len)

    def bf_check(self, vec, seq_len,  mean_mask_=None, weight_mean=None, std_mask_=None, weight_unbaised=None, label=1):
        line0 = torch.zeros(torch.Tensor(seq_len).shape).unsqueeze(0)
        line1 = torch.ones(torch.Tensor(seq_len).shape).unsqueeze(0)
