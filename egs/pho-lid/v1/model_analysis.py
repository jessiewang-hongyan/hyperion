...
class Shared_TDNN(nn.Module):
    def __init__(self, input_dim, dropout=0.1):
        super(Shared_TDNN, self).__init__()
        self.dropout = nn.Dropout(p=dropout)
        self.conv1 = nn.Conv1d(in_channels=input_dim, out_channels=512, kernel_size=1)
        self.conv2 = nn.Conv1d(in_channels=512, out_channels=512, kernel_size=1)
        self.conv3 = nn.Conv1d(in_channels=512, out_channels=512, kernel_size=1)

        self.bn1 = nn.BatchNorm1d(512, momentum=0.1, affine=True)
        self.bn2 = nn.BatchNorm1d(512, momentum=0.1, affine=True)
        self.bn3 = nn.BatchNorm1d(512, momentum=0.1, affine=True)
    ...
    def forward(self, x):
        ...
        z2 = self.relu(self.conv2(norm_z1))
        norm_z2 = self.bn2(z2)

        z3 = self.relu(self.conv2(norm_z2))
        norm_z3 = self.bn2(z3)

        return {"z1": norm_z1, "z2": norm_z2, "z3": norm_z3}
...
class PHOLID(nn.Module):
    def __init__(self,input_dim, feat_dim,
        ...
        self.n_heads = n_heads
        self.feat_dim = feat_dim
        self.shared_TDNN = Shared_TDNN(input_dim, dropout)
        # self.shared_TDNN = nn.Sequential(nn.Dropout(p=dropout),
        #                                  nn.Conv1d(in_channels=input_dim, out_channels=512, kernel_size=1),
        #                                  nn.ReLU(),
    ...
    def forward(self, x, seq_len, mean_mask_=None, weight_mean=None, std_mask_=None, weight_unbaised=None,
                atten_mask=None, eps=1e-5, tdnn_layer='z3'):
        batch_size = x.size(0)
        T_len = x.size(1)
        x = x.view(batch_size * T_len, -1, self.input_dim).transpose(-1, -2)
        x = self.shared_TDNN(x)[tdnn_layer]
    ...
    def get_embeddings(self, x, seq_len, atten_mask=None, tdnn_layer='z3'):
        # self.eval()
        batch_size = x.size(0)
        T_len = x.size(1)
...
class PHOLID_conv_pho(PHOLID_conv):
    ...
    def forward(self, x, seq_len, mean_mask_=None, weight_mean=None, std_mask_=None, weight_unbaised=None,
            atten_mask=None, eps=1e-5, norm_tac=True, tdnn_layer='z3'):
        batch_size = x.size(0)
        T_len = x.size(1)
        x = x.view(batch_size * T_len, -1, self.input_dim).transpose(-1, -2)
        x = self.shared_TDNN(x)[tdnn_layer]
    ...
    def get_embeddings(self, x, seq_len, atten_mask=None, norm_pho=True, norm_tac=True, tdnn_layer='z3'):
        # self.eval()
        batch_size = x.size(0)
        T_len = x.size(1)
    ...
    def get_pho_embeddings(self, x, seq_len, atten_mask=None, norm_pho=True, tdnn_layer='z3'):
        batch_size = x.size(0)
        T_len = x.size(1)
