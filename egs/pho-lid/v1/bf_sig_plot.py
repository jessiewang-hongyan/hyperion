...
def plot(x, label, name, force_range=False):
    # print(x.shape)
    # print(label.shape)
    # print(x[:, 0].shape)
    # print(x[:, 1].shape)
...
def main():
    ...
    plot_path = "./plots/ppho_seame_bf_0205_clf_english_tuned_reg0_lr1e-05/"
    ...
    for epoch in tqdm(range(total_epochs)):
        ...
        # model_name = "./models/sig/ppho_seame_sig_tune_row29/ppho_seame_sig_tune_row29_reg10_lr1e-05_epoch_{}.ckpt".format(str(epoch))
        model_name = "./models/sig/ppho_seame_bf_0205_clf_english_tuned/ppho_seame_bf_0205_clf_english_tuned_reg0_lr1e-05_epoch_{}.ckpt".format(str(epoch))
        significance_model.load_state_dict(torch.load(model_name))
        significance_model.eval()
        model.eval()
        ...
        for step, (utt, labels, seq_len) in enumerate(train_data):
            ...
            outputs, raw_scores, scores, h_cat, h_repr = significance_model(utt_, new_seq_len, atten_mask=atten_mask, return_score=True)

            labels = labels.reshape(batch_size*frame_size)
            outputs = outputs.reshape(-1, outputs.shape[-1]).detach().cpu().numpy()
            raw_scores = raw_scores.reshape(-1, raw_scores.shape[-1]).detach().cpu().numpy() 
            scores = scores.reshape(-1, scores.shape[-1]).detach().cpu().numpy()
            h_cat = h_cat.reshape(-1, h_cat.shape[-1]).detach().cpu().numpy()
            # h_repr = h_repr.reshape(-1, h_repr.shape[-1]*h_repr.shape[-2]).detach().cpu().numpy()
            h_repr = h_repr.reshape(-1, h_repr.shape[-1]).detach().cpu().numpy()

            # remove paddings
            for o, r, s, hc, hr, lab in zip(outputs, raw_scores, scores, h_cat, h_repr, labels):
                if lab != 100:
                    output_samples.append(o)
                    raw_scores_samples.append(r)
                    s_samples.append(s)
                    h_cat_samples.append(hc)
                    h_repr_samples.append(hr)
                    label_samples.append(lab)
            if step > 5:
                break

        output_samples = np.array(output_samples)
        raw_scores_samples = np.array(raw_scores_samples)
        s_samples = np.array(s_samples)
        h_cat_samples = np.array(h_cat_samples)
        h_repr_samples = np.array(h_repr_samples)
        label_samples = np.array(label_samples)

        # print(f"output_samples: {output_samples.shape}")
        # print(f"raw_scores_samples: {raw_scores_samples.shape}")
        # print(f"s_samples: {s_samples.shape}")
        # print(f"h_cat_samples: {h_cat_samples.shape}")
        # print(f"h_repr_samples: {h_repr_samples.shape}")
        # print(f"label_samples: {label_samples.shape}")

        # pca
        pca = PCA(n_components=20)
        h_cat_pca = pca.fit_transform(h_cat_samples)
        h_repr_pca = pca.fit_transform(h_repr_samples)

        # t-SNE
        tsne = TSNE(n_components=2, learning_rate='auto', perplexity=40, n_iter=300)
        h_cat_tsne = tsne.fit_transform(h_cat_pca)
        h_repr_tsne = tsne.fit_transform(h_repr_pca)
        ...
        plot(h_repr_tsne, label_samples, plot_path_e + "/{}.png".format("h_repr"), force_range=True)
        plot(raw_scores_samples, label_samples, plot_path_e + "/{}.png".format("raw_scores"))
        plot(s_samples, label_samples, plot_path_e + "/{}.png".format("scores"))
