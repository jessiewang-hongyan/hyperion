...
{ bn=$1; sub(/.*\//,"",bn); sub(/\.mp3$/,"",bn);
  split(bn, parts, "_");
  print bn, "ffmpeg -i "$1" -f wav /export/fs05/ywang793/hyperion/egs/malawi/v1/data/wav/"bn".wav | /export/fs05/ywang793/hyperion/egs/malawi/v1/data/wav/"bn".wav" }' | sort -k1,1 > $data_dir/wav.scp
